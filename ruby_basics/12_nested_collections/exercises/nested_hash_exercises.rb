def find_language_information(languages, language_name)
  ...
  # can't just put in languages name straight as it needs to be a symbol which means adding symbol start and name together
end
...
def find_beautiful_languages(languages)
  ...
  language_hash = languages.select{|name, data| data[:is_beautiful?] == true}
  language_hash
end
...
def find_language_facts(languages, language_name, fact_index = 0)
  ...
  # I forgot about accessing the facts array THEN the facts_index
end
